...
import { HVACSystemSelector } from './HVACSystemSelector';
import { EnhancedHVACSystemSelector } from './EnhancedHVACSystemSelector';
import { ToggleLeft, ToggleRight, Sparkles } from 'lucide-react';

interface HVACSystemSelectorWrapperProps {
  coolingLoad?: number;
  heatingLoad?: number;
  roomArea?: number;
}

export function HVACSystemSelectorWrapper(props: HVACSystemSelectorWrapperProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Component Display */}
      {useEnhanced ? (
        <EnhancedHVACSystemSelector {...props} />
      ) : (
        <HVACSystemSelector {...props} />
      )}
    </div>
  );
}