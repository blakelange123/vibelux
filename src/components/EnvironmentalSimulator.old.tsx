...
export function EnvironmentalSimulator() {
    ...
    for (let hour = 0; hour < 24; hour++) {
      ...
      if (hour >= settings.lightingSchedule.startHour && hour < settings.lightingSchedule.endHour) {
        ...
        if (hour < settings.lightingSchedule.startHour + rampHours) {
          ...
          lightIntensity = settings.lightingSchedule.maxIntensity * progress;
          ...
          // Full intensity
          lightIntensity = settings.lightingSchedule.maxIntensity;
        }
  ...
  const updateEnvironmentalState = () => {
    ...
    const lightingPower = (currentSchedule.lightIntensity / 100) * lightingPowerDensity * roomArea;
  ...
  const calculateResults = (): SimulationResults => {
    ...
    // Input costs (daily)
    const electricityCostPerKWh = 0.12; // $/kWh
    const electricityCost = energyConsumption.total * electricityCostPerKWh;
    
    const nutrientCostPerPlant = 0.15; // $/plant/cycle
    const dailyNutrientCost = (nutrientCostPerPlant * totalPlants) / cropParams.cycleDays;
    
    const waterCostPerLiter = 0.003; // $/L
    const dailyWaterCost = avgTranspiration * roomArea * waterCostPerLiter;
    
    const laborCostPerDay = 25; // $/day for maintenance
    
    const totalDailyCost = electricityCost + dailyNutrientCost + dailyWaterCost + laborCostPerDay;
    ...
    const projectedRevenue = totalProjectedYield * cropParams.marketPrice;
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-gray-400 text-lg max-w-2xl mx-auto">
          Complete 24-hour environmental simulation with dynamic modeling and optimization
        </p>
      </div>
      ...
      {showSettings && (
        <div className="bg-gray-900/50 backdrop-blur-xl rounded-xl p-6 border border-gray-800">
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            {/* Facility Settings */}
            <div>
              <h4 className="text-sm font-medium text-gray-300 mb-3">Facility</h4>
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  <label className="text-xs text-gray-400">Max Intensity (PPFD)</label>
                  <input
                    type="number"
